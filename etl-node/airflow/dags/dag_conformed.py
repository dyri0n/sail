...
# --- CONFIGURACIÓN ---
# Nombre de la conexión configurada en Airflow Admin -> Connections
# Debe apuntar a tu DWH (PostgreSQL)
...
with DAG(
    ...
    schedule_interval="@daily",  # Se ejecuta una vez al día (ej: 02:00 AM)
    catchup=False,
    ...
    t_api_feriados = DockerOperator(
        ...
        api_version='auto',
        auto_remove=True,
        docker_url=settings.get_worker_api_url(), # Usa config centralizada
        network_mode='host',
        environment={
            'DB_URI': settings.get_staging_uri()
        },
    ...
    t_dim_cargo = SQLExecuteQueryOperator(
        task_id="merge_dim_cargo", conn_id=DB_CONN_ID, sql="dimensiones/dim_cargo.sql"
    )
    ...
    t_dim_ceco = SQLExecuteQueryOperator(
        task_id="merge_dim_ceco", conn_id=DB_CONN_ID, sql="dimensiones/dim_ceco.sql"
    )
    ...
    t_dim_modalidad = SQLExecuteQueryOperator(
        ...
        conn_id=DB_CONN_ID,
        sql="dimensiones/dim_modalidad.sql",
    )
